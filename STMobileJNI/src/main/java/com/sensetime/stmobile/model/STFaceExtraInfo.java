...
public class STFaceExtraInfo {
    ...
    public STPoint[] all106 = new STPoint[106 * STCommon.ST_MOBILE_HUMAN_ACTION_MAX_FACE_COUNT];
    ...
    public STPoint[] getAll106(int index){
        if(index > allCount - 1 || index < 0){
            return null;
        }

        STPoint[] points = new STPoint[106];
        for(int i = 0; i < 22; i++){
            points[i] = all106[106 *index + i];
        }
        return points;
    }
