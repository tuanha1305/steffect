...
JNIEXPORT jint JNICALL Java_com_sensetime_stmobile_STMobileAuthentificationNative_checkActiveCode(JNIEnv * env, jobject obj, jobject context, jstring licensePath, jstring activationCode);
...
//    const char *targetProductName = env->GetStringUTFChars(productName, 0);
    ...
    int res = st_mobile_check_activecode(env, context, targetLicensePath, targetActivationCode);
...
//    int license_size = licenseSize;
    int res = st_mobile_check_activecode_from_buffer(env, context, targetLicenseBuffer, licenseSize, activationCodeString);
