...
//                            //在屏幕画点
                            ...
                            Bitmap bitmapjiemao = BitmapFactory.decodeResource(mContext.getResources(), R.mipmap.kuazhangjiemao);
                            Bitmap bitmapYanying = BitmapFactory.decodeResource(mContext.getResources(), R.mipmap.quanbaoyanying);
                            Bitmap bitmapYanxian = BitmapFactory.decodeResource(mContext.getResources(), R.mipmap.wuguyanxain);

                            int textYanxian = OpenGLUtils.NO_TEXTURE;
                            int textYanying = OpenGLUtils.NO_TEXTURE;
                            ...
                            textYanxian = OpenGLUtils.loadTexture(bitmapYanxian,textYanxian, true);
                            textYanying = OpenGLUtils.loadTexture(bitmapYanying,textYanying,true);
                            for(int i =0;i<arrayFaces.length;i++){
                                ...
                                drawRightJiemao(stPoint240,textYanxian);
                                drawRightJiemao(stPoint240,textYanying);

                                float _mousecolors[] = {0.5f,0.5f,0.5f,0.5f};
...
//    [downMouseBeAdd removeAllObjects];downMouse = nil;
    ...
    public void drawUPMouSe(STPoint[] points,float mousecolors[]) {
        ...
        for (int i = 0; i <pointMouseList.size();
            ...
            squareVertices2[i * 8 + 1] = mousecolors[1];
            squareVertices2[i * 8 + 2] = mousecolors[2];
//        squareVertices2[_index*8+3] = _mousecolors[3]/5.0;
...
//    glHint(GL_LINE_SMOOTH_HINT, GL_NICEST);
        ...
        for (int i = 0; i <pointMouseList.size();
             i++){
            squareVertices2[i * 8 + 3] = mousecolors[3];
//        squareVertices2[_index*8+7] = _mousecolors[3]/5.0;
